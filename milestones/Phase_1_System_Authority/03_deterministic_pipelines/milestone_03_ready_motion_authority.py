...
AUTH_FILE = RUNTIME_DIR / "ready_authority.json"
...
def arm_ready():
    ...
    _audit("AUTHORIZATION_ARMED", routine="ready_pose", sha256=permit["sha256"])
    return permit
...
def execute_ready():
    ...
    try:
        ...
        return {"ok": False, "authorized": False,
                "error": "Local Home authorization is not armed",
                "hardware_action": "NONE"}

    age = time.time() - float(permit.get("timestamp_unix", 0))
    if permit.get("scope") != "ONE_READY_POSE" or age < 0 or age > AUTH_MAX_AGE_SECONDS:
        AUTH_FILE.unlink(missing_ok=True)
        return {"ok": False, "authorized": False,
                "error": "Home authorization invalid or expired",
                "hardware_action": "NONE"}

    if permit.get("sha256") != _sha256(THIS_FILE):
        AUTH_FILE.unlink(missing_ok=True)
        return {"ok": False, "authorized": False,
                "error": "Home authority code changed after arming",
                "hardware_action": "NONE"}
    ...
    try:
        AUTH_FILE.unlink(missing_ok=True)
        _audit("MOTION_AUTHORIZATION_CONSUMED", routine="ready_pose")
        ...
        try:
            ...
            _audit("MOTION_STARTED", routine="ready_pose", command=cmd)
        ...
        _audit("MOTION_FINISHED", routine="ready_pose", final_state=final_state)
        ...
        return {
            ...
            "hardware_action": "READY_POSE_EXECUTED",
        }
    finally:
