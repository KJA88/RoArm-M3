...
AUTH_FILE = RUNTIME_DIR / "home_authority.json"
...
def arm_home():
    ...
    _audit("AUTHORIZATION_ARMED", routine="home_init", sha256=permit["sha256"])
    return permit
...
def execute_home():
    ...
    if permit.get("scope") != "ONE_HOME_INIT" or age < 0 or age > AUTH_MAX_AGE_SECONDS:
    ...
    try:
        AUTH_FILE.unlink(missing_ok=True)
        _audit("MOTION_AUTHORIZATION_CONSUMED", routine="home_init")
        ...
        try:
            ...
            cmd = {"T": 100}
            _audit("MOTION_STARTED", routine="home_init", command=cmd)
        ...
        _audit("MOTION_FINISHED", routine="home_init", final_state=final_state)
        ...
        return {
            ...
            "hardware_action": "HOME_INIT_EXECUTED",
        }
    finally:
