...
AUTH_FILE = RUNTIME_DIR / "observe_right_authority.json"
...
def execute_observe_right():
    ...
    try:
        ...
        return {
            "ok": False,
            "authorized": False,
            "error": "Local Observe Right authorization is not armed",
            "hardware_action": "NONE"
        }

    age = time.time() - float(permit.get("timestamp_unix", 0))
    if permit.get("scope") != "ONE_OBSERVE_RIGHT_POSE" or age < 0 or age > AUTH_MAX_AGE_SECONDS:
        AUTH_FILE.unlink(missing_ok=True)
        return {
            "ok": False,
            "authorized": False,
            "error": "Observe Right authorization invalid or expired",
            "hardware_action": "NONE"
        }
    ...
    if permit.get("sha256") != _sha256(THIS_FILE):
        ...
        return {
            "ok": False,
            "authorized": False,
            "error": "Observe Right authority code changed after arming",
            "hardware_action": "NONE"
        }
    ...
    try:
        ...
        return {
            ...
            "routine": "OBSERVE_RIGHT",
            "final_state": final_state,
            "hardware_action": "OBSERVE_RIGHT_EXECUTED",
        }
