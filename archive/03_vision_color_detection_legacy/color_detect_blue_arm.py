#!/usr/bin/env python3
"""
Capture one frame and find the arm's blue/purple plastic.

What it does:
  - Captures a frame from the IMX708
  - Converts to HSV
  - Thresholds for blue/purple (tuned for your camera)
  - Picks the largest contour
  - Computes centroid (cx, cy)
  - Computes offset from image center (dx, dy)
  - Draws a marker and saves blue_arm_out.jpg
"""

from picamera2 import Picamera2
import cv2
import numpy as np
import time


def main():
    picam2 = Picamera2()
    config = picam2.create_still_configuration()
    picam2.configure(config)

    print("Starting camera...")
    picam2.start()
    time.sleep(2.0)

    print("Capturing frame...")
    frame = picam2.capture_array()
    print("Frame shape:", frame.shape)

    # Downscale for speed
    scale = 0.5
    frame_small = cv2.resize(
        frame,
        (int(frame.shape[1] * scale), int(frame.shape[0] * scale)),
        interpolation=cv2.INTER_AREA,
    )

    hsv = cv2.cvtColor(frame_small, cv2.COLOR_BGR2HSV)

    # ---- BLUE / PURPLE RANGE (tweak if needed) ----
    # Hue for blue is ~100–140, purple a bit higher.
    # We also require some saturation and brightness.
    lower_blue = np.array([90, 60, 40], dtype=np.uint8)
    upper_blue = np.array([150, 255, 255], dtype=np.uint8)

    mask = cv2.inRange(hsv, lower_blue, upper_blue)

    kernel = np.ones((5, 5), np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)

    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Ignore tiny blobs
    contours = [c for c in contours if cv2.contourArea(c) > 500]

    if not contours:
        print("No blue-ish object found. Try moving the printed arm closer or into view.")
        out_name = "blue_arm_out.jpg"
        cv2.imwrite(out_name, frame_small)
        print(f"Saved raw frame to {out_name}")
        picam2.close()
        return

    largest = max(contours, key=cv2.contourArea)
    area = cv2.contourArea(largest)
    print("Largest contour area:", area)

    M = cv2.moments(largest)
    if M["m00"] == 0:
        print("Moments area zero, skipping.")
        out_name = "blue_arm_out.jpg"
        cv2.imwrite(out_name, frame_small)
        print(f"Saved frame without markups to {out_name}")
        picam2.close()
        return

    cx = int(M["m10"] / M["m00"])
    cy = int(M["m01"] / M["m00"])

    h, w = frame_small.shape[:2]
    center_x = w // 2
    center_y = h // 2

    dx = cx - center_x
    dy = cy - center_y

    print(f"Detected blue object at (cx, cy) = ({cx}, {cy}) in resized image.")
    print(f"Image center = ({center_x}, {center_y})")
    print(f"Offset from center: dx = {dx}, dy = {dy} (pixels)")

    full_cx = int(cx / scale)
    full_cy = int(cy / scale)
    print(f"Approx. full-res coordinates: ({full_cx}, {full_cy})")

    # Draw marker
    cv2.circle(frame_small, (cx, cy), 10, (0, 255, 0), 2)
    cv2.drawMarker(
        frame_small,
        (cx, cy),
        (0, 255, 0),
        markerType=cv2.MARKER_CROSS,
        markerSize=20,
        thickness=2,
    )

    out_name = "blue_arm_out.jpg"
    cv2.imwrite(out_name, frame_small)
    print(f"Saved debug image with marker to {out_name}")

    picam2.close()
    print("Done.")


if __name__ == "__main__":
    main()
